...
$$\large\micro = e^{\,\int\!f(x)dx}$$
#### يكون حل المعادلة:
$$y\,\micro = \int\micro\,g(x)dx$$
### ملاحظة:
اذا كانت المعادلة:
...
### خطوات
#### 1. بقسمة المعادلة على $y^n$ $$y^{-n} \dfrac{dy}{dx} + f(x)\,y^{1-n} = g(x)$$
#### 2. بفرض: $$u = y^{1-n} \quad,\quad \dfrac{du}{dx} = (1-n)\,y^{-n}\dfrac{dy}{dx} $$
#### 3. بالتعويض:
